<template>
    <div class="relative" :class="[label ? 'pt-4' : '']">
        <label class="absolute top-1.5 left-4 px-1.5 py-0" :class="bgClass" v-show="(label && !placeholder) || (label && placeholder && (tagsLocal.length > 0 || inputVal.length > 0))" :for="id">
            {{ label }}
        </label>
...
const bgClassInverted = computed(() => {
    return props.bgClass === 'bg-secondary' ? 'bg-primary' : 'bg-secondary'
})
